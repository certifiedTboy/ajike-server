...
export class AppEvents extends EventEmitter {
    ...
    emitEvent(name, eventData) {
        ...
        if (this.activeJobs.has(eventData.id)) {
            logger.info(`Job for ${eventData?.id} is already in the queue. Skipping.`);
            return;
        }
        // emit events
    ...
    calculateJobPattern(eventData) {
        ...
        logger.info(`Add event to queue for ${eventData.id} to run at ${scheduledTime.toLocaleTimeString()}`);
    ...
    async runEvent(name, eventData) {
        switch (name) {
            case "new-user":
                ...
                // );
                await EmailService.sendEmailWithLambda(eventData?.email, "Welcome! Verify Your Account", "create-account", { name: eventData.firstName, otp: eventData.otp });
            ...
            case "user-verified":
                ...
                // );
                await EmailService.sendEmailWithLambda(eventData?.email, "Account Verified!", "account-verified", { name: eventData.firstName });
                logger.info(`Verification email sent to ${eventData.email}`);
                break;
            case "password-reset":
                // await EmailService.sendEmail(
                ...
                //   { name: eventData.firstName, otp: eventData.otp },
                // );
                await EmailService.sendEmailWithLambda(eventData?.email, "Password Reset Request", "password-reset", { name: eventData.firstName, otp: eventData.otp });
            ...
            case "password-changed":
                ...
                //   { name: eventData.firstName },
                // );
                await EmailService.sendEmailWithLambda(eventData?.email, "Your Password Has Been Changed", "password-changed", { name: eventData.firstName });
                logger.info(`Password changed confirmation sent to ${eventData.email}`);
                break;
            case "create-new-service":
                if (eventData?.serviceData) {
                    const newService = await ServiceServices.createService(eventData.serviceData);
                    logger.info(`new service with id: ${newService?._id.toString()} completed`);
                }
                break;
            case "update-new-service":
                if (eventData?.serviceData) {
                    await ServiceServices.updateService(eventData?.serviceId, eventData.serviceData, eventData?.serviceData?.user?._id);
                    logger.info(`service with id: ${eventData?.serviceId.toString()} has been updated`);
                }
                break;
            case "add-service-feedback":
                if (eventData?.feedbackData && eventData?.serviceId) {
                    await ServiceServices.giveFeedback(eventData?.serviceId, eventData?.userId, eventData?.feedbackData?.text, eventData?.feedbackData?.rating, eventData?.feedbackData?.updateServiceData);
                ...
                if (eventData?.serviceData && eventData?.serviceId) {
                    ...
                    if (EMAIL_FROM) {
                        ...
                        await EmailService.sendEmailWithLambda("etosin70@gmail.com", "New Service request", "new-service", {
                            ...
                            propertyType: data?.propertyType || "Not Provided",
                            category: data?.category || "Not Provided",
                            serviceLocation: `${data?.serviceCity}, ${data?.serviceState}` ||
                                "Not Provided",
                            plan: data?.plan || "Not Provided",
                            budget: data?.budget || "Not Provided",
                            address: data?.address || "Not Provided",
                        ...
                        logger.info(`New service request update has been sent to ${EMAIL_FROM}`);
                    }
                    if (user) {
                        // await EmailService.sendEmail(
                        //   [user?.email],
                        ...
                        //   },
                        // );
                        await EmailService.sendEmailWithLambda(user?.email, "Your Service request has been received", "new-service-user", {
                            title: data?.title || "New cleaning service",
                            status: data?.status || "New",
                            name: user?.firstName + " " + user?.lastName,
                            dashboardURl: `${CLIENT_URL}/dashboard/services/${data?._id}`,
                        });
                        logger.info(`New service request update has been sent to ${user?.email}`);
                    }
                }
                break;
            case "update-service":
                if (eventData?.serviceData && eventData?.serviceId) {
                    ...
                    if (user) {
                        ...
                        //   },
                        // );
                        await EmailService.sendEmailWithLambda(user?.email, "Your service has a new status", "update-service", {
                            title: data?.title,
                            status: data?.status,
                            category: data?.category,
                            propertyType: data?.propertyType,
