...
export class UserServices {
    ...
    static async passwordResetRequest(email) {
        ...
        const newUser = await this.updateUserData({ email: userExist.email }, {
            $set: {
                otp: AppHelpers.generateOTP(),
                otpExpiry: new Date(Date.now() + 60 * 60 * 1000),
                isVerified: false,
    ...
    static async updatePassword(otp, password) {
        ...
        if (!userExist)
            throw new HttpException(404, "user with email does not exist");
        const updatedUser = await this.updateUserData({ otp: userExist.otp }, {
            isVerified: true,
    ...
    static async createGoogleUser(userData) {
        ...
        if (userExist && !userExist?.picture) {
            return await this.updateUserData({ email: userExist?.email }, {
