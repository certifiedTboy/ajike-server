...
import { SERVICES, SERVICE_DETAILS } from "../service/dummy-services.js";
...
const listServicesTool = tool(async ({ type }) => {
    const services = SERVICES.filter((service) => !type || service.type === type);
    return JSON.stringify(services.map(({ id, name, type: audience, price, detail, slug }) => ({
        id,
        name,
        audience,
        price,
    ...
    description: "List Ajike pest-control and cleaning services, including starting prices and short descriptions.",
    schema: z.object({
        type: z
...
const getServiceDetailsTool = tool(async ({ serviceId }) => {
    ...
    description: "Get the authoritative details for one Ajike service, including audience, included work, duration, preparation, and next steps.",
    schema: z.object({
        serviceId: z
            .string()
...
const requestHumanSupportTool = tool(async ({ reason }) => JSON.stringify({
    requested: true,
    reason,
    message: "A human support specialist can help with this request. Ask the user to select the Speak with human support button.",
}), {
    name: "request_human_support",
    description: "Use when the user asks for a person, needs an appointment or exact quote, or the request requires information unavailable to the assistant.",
    schema: z.object({
...
export async function answerWithAjikeAssistant(message, history = []) {
    if (!agent) {
...
export function formatAiAssistantResponse(text, room) {
    return {
        text,
        sender: "Ajike AI",
        ...(room ? { room } : {}),
